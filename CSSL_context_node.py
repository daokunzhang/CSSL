...
parser = argparse.ArgumentParser(description='context node prediction based CSSL for link prediction')
...
negative_samples = np_random.choice(
    list(graph_train.nodes()), size=len(positive_pairs), p=sampling_distribution_norm
)
negative_pairs = [
    (positive_pairs[i][0], negative_samples[i], 0)
...
for i in range(args.epochs):
    ...
    for j in range(batches):
        ...
        sampled_end1_dst = np.array([graph_train_node_list.index(node) for node in sampled_end1_dst])
        ...
        sampled_end2_dst = np.array([graph_train_node_list.index(node) for node in sampled_end2_dst])
